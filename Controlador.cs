...
public class Controlador : MonoBehaviour
{
    ...
    public float multiDanioUlti = 1;
    public float multiDanioAtaqueEspecial = 1;
    ...
    [PunRPC]
    public void recibirdaniorpc(int d, string tipo, bool bStun, int id)
    {
        RecibirDanio(d, tipo, bStun, id);
        Debug.Log("    1   +    " + id);

    }
    [PunRPC]
    public virtual void RecibirDanio(int d, string tipo, bool bStun, int id)
    {
    ...
    public void habilidadesYacciones()
    {
        ...
        if (!haciendoAccion)
        {
            ...
            {
               // recibirdaniorpc(95, "guay", false, photonView.ViewID);
            }
        ...
        if (player != null)
        {
            ...
            if (Physics.Raycast(ray, out trans))
            {
                if (trans.collider != null)
                {
                    ...
                    {
                        // No hay nada entre la c�mara y el jugador
                        if (_fader != null)
                        {
                            _fader.doFade = false;
                        }
