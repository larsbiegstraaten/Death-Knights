...
public class rotacionCamara : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // Calcula la dirección hacia donde mirar
        Vector3 lookDirection = cursorPos - transform.position;

        // Haz que el personaje mire hacia la dirección del cursor
        transform.forward = lookDirection.normalized;
        

        // Opcional: Limita la rotación en ciertos ejes si es necesario
        Vector3 newEulerAngles = transform.eulerAngles;
        newEulerAngles.y = 0f; // Mantén la rotación en el eje X en 0 grados
        newEulerAngles.x = 0f; // Mantén la rotación en el eje Z en 0 grados
